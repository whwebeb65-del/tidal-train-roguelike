...
export function renderSocialHubView(model: SocialHubViewModel): string {
  if (!model.legionId) {
    return `<section class="deferred-section living-zone lighthouse-dock">
      ...
      <div class="support-platform"><button class="primary" data-action="join-legion">加入「潮汐灯塔团」</button></div>
    </section>`;
  }
  ...
  const supports = model.supports.map((support) => `<button class="support-card ${support.selected ? 'selected' : ''}" data-action="toggle-support" data-support-id="${support.id}">
  ...
  return `<section class="deferred-section living-zone lighthouse-dock">
    ...
    <div class="expedition-progress expedition-chart"><div><span>本周远征贡献</span><b>${model.contribution} / 100</b></div><div class="progress"><i style="width:${Math.min(100, model.contribution)}%"></i></div></div>
    <div class="expedition-milestones beacon-line">${milestones}</div>
    <div class="section-title crew-line"><h2>异步列车队</h2><span>${model.supports.filter((support) => support.selected).length}/2 名支援 · ${selectedEffects}</span></div>
