...
export function renderCaptainSelection(): string {
  const cards = CAPTAIN_CATALOG.map((captain) => {
    ...
    return `<button class="captain-choice-card captain-berth" data-action="select-captain" data-captain-id="${captain.id}">
  ...
  return `<section class="captain-select scene living-zone captain-platform">
    <div class="platform-departure-sign">
      <span class="eyebrow">FIRST DEPARTURE</span>
      <div><h1>选择你的列车长</h1><p>选择只影响形象与技能演出，基础能力完全一致。之后可在车站随时切换，并为每位列车长收集可叠加属性的皮肤。</p></div>
      <b>候车月台 · 01</b>
    </div>
